...
describe('ProfileBanner', () => {
  it('renders name, target capital and yield presets', () => {
    ...
    expect(screen.getByRole('progressbar', { name: 'Прогресс к цели' })).toBeInTheDocument()
    expect(screen.getByText(/Осталось/)).toBeInTheDocument()
    expect(screen.getByText(/Изъятие после 50/)).toBeInTheDocument()
    expect(screen.getByText('Базовый взнос')).toBeInTheDocument()
    expect(screen.getByText('Срок накоплений')).toBeInTheDocument()
